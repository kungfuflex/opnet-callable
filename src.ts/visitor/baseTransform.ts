...
export class BaseTransformVisitor extends AbstractTransformVisitor<Node> {
  ...
  visitFunctionTypeNode(node: FunctionTypeNode): FunctionTypeNode {
    ...
    node.explicitThisType = this.visit(
      node.explicitThisType,
    ) as NamedTypeNode | null;
    return node;
  }
  ...
  visitClassDeclaration(
    ...
    node.extendsType = this.visit(node.extendsType) as NamedTypeNode;
    node.implementsTypes = this.visit(node.implementsTypes) as
      | NamedTypeNode[]
      | null;
    this.depth++;
  ...
  visitImportStatement(node: ImportStatement): ImportStatement {
    node.namespaceName = this.visit(
      node.namespaceName,
    ) as IdentifierExpression | null;
    node.declarations = this.visit(node.declarations) as
      | ImportDeclaration[]
      | null;
    return node;
  }
  ...
  visitInterfaceDeclaration(
    ...
    node.name = this.visit(node.name) as IdentifierExpression;
    node.decorators = this.visit(node.decorators) as DecoratorNode[] | null;
    node.typeParameters = this.visit(node.typeParameters) as
      | TypeParameterNode[]
      | null;
    node.implementsTypes = this.visit(node.implementsTypes) as
      | NamedTypeNode[]
