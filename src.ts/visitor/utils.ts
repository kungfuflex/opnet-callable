...
export function decoratorName(node: DecoratorNode): string {
  return (<IdentifierExpression>node.name).text;
}
...
export function isEntry(node: Node): boolean {
  return (
    isUserEntry(node) || node.range.source.sourceKind == SourceKind.LibraryEntry
  );
}

export function className(
  _class: ClassDeclaration | InterfaceDeclaration | FunctionDeclaration,
): string {
...
export function isMethodNamed(
  name: string,
): (_: DeclarationStatement) => boolean {
  return (stmt: DeclarationStatement) =>
    stmt.kind == NodeKind.MethodDeclaration && toString(stmt.name) === name;
}
