...
export class Parameter {
  ...
  static fromNode(node: ParameterNode): Parameter {
    return new Parameter(toString(node.name), getTypeName(node.type));
  }
}
...
export class CallableMethod {
  ...
  constructor(name: string, parameters: string[], returnType: string) {
    this.name = name;
  ...
  static parametersFromSignature(node: FunctionTypeNode): string[] {
...
export class CallableClass {
  ...
  static fromNode(node: DeclarationStatement): CallableClass {
